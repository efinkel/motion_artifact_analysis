function [fig, rms_summary] = plotPreStimMove(mouse_name, filtSignal, time, a, range)
...
for m = 1:2
    ...
    for i=1:numel(trial_types)
        ...
        for j = 1:numel(trial_starts)
            ...
            patch([0,stim_len,stim_len,0], [offset*count - 0.075, offset*count - 0.075,...
                offset*count + 0.075, offset*count + 0.075],...
                stim_col,'edgecolor', stim_col);
            line_y = offset*count;
            line_heights = [line_heights, line_y];
            plot([range(1):1/30000:range(2)], windows{j}+line_y, 'k')
            plot([stim_len-1/30000:1/30000:range(2)], post_stim{j}(stim_len*30000:end)+offset*count,...
                'color', line_colors{i})
            count = count + 1;
        end
        count = count + 2;
    end
    ylim([-0.15*2, 0.15*(count+3)])
